...
// Prepara a consulta SQL para buscar o diagnóstico específico
$stmt = $conn->prepare("SELECT * FROM diagnosticos WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();

// Verifica se encontrou o diagnóstico
if ($result->num_rows > 0) {
  // Retorna os dados do diagnóstico em formato JSON
  echo json_encode($result->fetch_assoc());
} else {
  // Retorna uma mensagem de erro em JSON caso o diagnóstico não seja encontrado
  echo json_encode(["mensagem" => "Diagnóstico não encontrado."]);
}
