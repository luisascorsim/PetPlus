...
$id_usuario_logado = (int)$_SESSION['id_usuario']; // Usuário logado
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($id_usuario_destino) || empty($titulo) || empty($mensagem_conteudo)) {
            ...
            if ($action === 'criar_notificacao') {
                ...
                $stmt->execute();
                $stmt->close();
                // Considere redirecionar para evitar reenvio do formulário ao atualizar
                header("Location: " . strtok($_SERVER["REQUEST_URI"], '?') . "?feedback=criada_ok");
                exit();
                ...
                if ($id_notificacao_edit) {
                    ...
                    if ($stmt->affected_rows > 0) {
                        // Considere redirecionar
                        header("Location: " . strtok($_SERVER["REQUEST_URI"], '?') . "?feedback=atualizada_ok");
                        exit();
                    } else {
    ...
    // Deletar Notificação 
    // Header tava dando erro aqui
    if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id']) && is_numeric($_GET['id'])) {
        $id_notificacao_excluir = (int)$_GET['id'];
        $sql_excluir = "DELETE FROM Notificacoes 
                        WHERE id_notificacao = ? 
                        AND (remetente_id_usuario = ? OR id_usuario = ?)";
        $stmt_excluir = $conn->prepare($sql_excluir);
        $stmt_excluir->bind_param("iii", $id_notificacao_excluir, $id_usuario_logado, $id_usuario_logado);
        $stmt_excluir->execute();

        if ($stmt_excluir->affected_rows > 0) {
            header("Location: " . strtok($_SERVER["REQUEST_URI"], '?') . "?feedback=excluida_ok");
        } else {
            header("Location: " . strtok($_SERVER["REQUEST_URI"], '?') . "?feedback=excluida_falha_logica_alterada");
        }
        $stmt_excluir->close();
        exit(); // Importante sair após o redirecionamento
    }

    // Populando o formulário para edição
    if (isset($_GET['action']) && $_GET['action'] === 'editar' && isset($_GET['id']) && is_numeric($_GET['id'])) {
        ...
        $sql_get_edit = "SELECT id_notificacao, id_usuario, titulo, mensagem 
                         FROM Notificacoes 
                         WHERE id_notificacao = ? AND remetente_id_usuario = ?"; // Apenas o remetente pode editar
...
// Se chegamos aqui, nenhum redirecionamento ocorreu. Agora podemos buscar dados para exibição.

// Feedback de status via GET (após redirecionamentos)
if (isset($_GET['feedback'])) {
    if ($_GET['feedback'] == 'criada_ok') {
        $mensagem_feedback = 'Notificação enviada com sucesso!';
        $tipo_feedback = 'sucesso';
    } elseif ($_GET['feedback'] == 'atualizada_ok') {
        $mensagem_feedback = 'Notificação atualizada com sucesso!';
        $tipo_feedback = 'sucesso';
    } elseif ($_GET['feedback'] == 'excluida_ok') {
        $mensagem_feedback = 'Notificação excluída com sucesso.';
        $tipo_feedback = 'sucesso';
    } elseif ($_GET['feedback'] == 'excluida_falha_logica_alterada') { 
        $mensagem_feedback = 'Não foi possível excluir a notificação. Ela pode não existir, ou você não é o remetente nem o destinatário dela.';
        $tipo_feedback = 'erro';
    }
}


// Buscar todos os usuários para o dropdown de destinatário
...
try {
    ...
    if(empty($mensagem_feedback)) {
        $mensagem_feedback = "Erro ao carregar lista de usuários: " . $e->getMessage();
        $tipo_feedback = "erro";
    }
}
...
try {
    ...
    if(empty($mensagem_feedback)) {
        $mensagem_feedback = "Erro ao carregar suas notificações enviadas: " . $e->getMessage();
        $tipo_feedback = "erro";
    }
}

// Coloquei aqui porque tava dando erro na hora de excluir a mensagem
    ...
    <div class="form-section">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                ...
                <select name="id_usuario_destino" id="id_usuario_destino" required>
                    ...
                    <?php foreach ($usuarios_para_select as $usuario): ?>
                        <option value="<?php echo htmlspecialchars($usuario['id_usuario']); ?>" 
                            <?php 
                                $selected_destinatario = '';
                                if ($edit_mode && $notificacao_para_editar && $notificacao_para_editar['id_usuario'] == $usuario['id_usuario']) {
                                    $selected_destinatario = 'selected';
                                } elseif (!$edit_mode && isset($_POST['id_usuario_destino']) && $_POST['id_usuario_destino'] == $usuario['id_usuario']) {
                                    $selected_destinatario = 'selected'; // Repopular em caso de erro no POST de criação
                                }
                                echo $selected_destinatario;
                            ?>>
            ...
            <div class="form-group">
                ...
                <input type="text" name="titulo" id="titulo" value="<?php echo $edit_mode && $notificacao_para_editar ? htmlspecialchars($notificacao_para_editar['titulo']) : (isset($_POST['titulo']) ? htmlspecialchars($_POST['titulo']) : ''); ?>" required>
            </div>

            <div class="form-group">
                <label for="mensagem">Mensagem:</label>
                <textarea name="mensagem" id="mensagem" rows="5" required><?php echo $edit_mode && $notificacao_para_editar ? htmlspecialchars($notificacao_para_editar['mensagem']) : (isset($_POST['mensagem']) ? htmlspecialchars($_POST['mensagem']) : ''); ?></textarea>
            </div>
                ...
                <?php foreach ($notificacoes_enviadas as $notificacao): ?>
                    <tr>
                        ...
                        <td class="actions">
                            ...
                            <a href="?action=excluir&id=<?php echo $notificacao['id_notificacao']; ?>" class="btn btn-danger" onclick="return confirm('Tem certeza que deseja excluir esta notificação?');">Excluir</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
